...
describe("Relay", () => {
  it("sends your text to the right Grok Bot bot and texts back its reply", async () => {
    ...
    h.grok.post("bot-chief", { id: "a1", role: "assistant", text: "**3 meetings** today:\n- 10:00 standup", status: "completed" });
    await h.step(1000);
  ...
  it("tags a reply when Messages would send it from another bot's address, and routes swipe-replies back", async () => {
    ...
    h.grok.post("bot-chief", { id: "c1", role: "assistant", text: "Here's your brief", done: true });
    await h.step(1000);
  ...
  it("doesn't replay what was already in the transcript, but relays new routine posts", async () => {
    const h = harness();
    h.grok.post("bot-dev", { id: "old", role: "assistant", text: "old news", done: true });
    await h.relay.start();
    await h.step(5000);
    assert.equal(h.sender.sent.length, 0);

    h.grok.post("bot-dev", { id: "r1", role: "assistant", text: "Nightly report: 2 agent PRs merged", done: true });
    await h.step(5000);
  ...
  it("with relay: replies, only answers to your texts are forwarded", async () => {
    ...
    h.grok.post("bot-chief", { id: "routine", role: "assistant", text: "Unprompted update", done: true });
    await h.step(5000);
    assert.equal(h.sender.sent.length, 0);

    h.text(CHIEF, "Now answer me");
    await h.step();
    h.grok.post("bot-chief", { id: "answer", role: "assistant", text: "Answer", done: true });
    await h.step(1000);
  ...
  it("waits for a streaming reply to finish before texting it", async () => {
    ...
    h.grok.post("bot-chief", { id: "s1", role: "assistant", text: "Work" });
    await h.step(1000);
    h.grok.post("bot-chief", { id: "s1", role: "assistant", text: "Working on it: found 3 sources" });
    await h.step(1000);
    await h.step(1000);
    assert.equal(h.sender.sent.length, 0);
    await h.step(1000);
  ...
  it("holds unprompted messages during quiet hours and sends them after", async () => {
    ...
    h.grok.post("bot-dev", { id: "late", role: "assistant", text: "Agent finished at 11pm", done: true });
    await h.step(5000);
    assert.equal(h.sender.sent.length, 0);
    ...
    await h.step();
    h.grok.post("bot-chief", { id: "reply", role: "assistant", text: "Yes", done: true });
    await h.step(1000);
  ...
  it("splits long replies and prefixes group messages with the speaking bot", async () => {
    ...
    h.grok.post("group-1", { id: "g1", role: "assistant", author: { name: "Researcher" }, text: long, done: true });
    await h.step(1000);
